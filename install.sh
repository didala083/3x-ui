...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        last_version=$(curl -Ls "https://api.github.com/repos/didala083/3x-ui/releases/latest" | grep '"tag_name":' | sed -E 's/.*"([^"]+)".*/\1/')
        ...
        last_version=$1
        url="https://github.com/didala083/3x-ui/releases/download/${last_version}/x-ui-linux-$(arch3xui).tar.gz"
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/didala083/3x-ui/main/x-ui.sh
